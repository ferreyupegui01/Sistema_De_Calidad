...
const ICON_MAP = {
    'folder': <FaFolder />,
    'ban': <FaBan />,            // Restricciones / Sin Alérgenos
...
const GestionAlergenos = () => {
    ...
    const handleCardClick = async (card) => {
        ...
        try {
            ...
            // 1. Encontrar carpeta raíz
            let rootId = await findOrCreateFolder(1, "Programa Alérgenos", headers);
    ...
    const fetchReportesOperativos = async () => {
        ...
        try {
            ...
            // Filtrar reportes para Alérgenos
            const filtered = data.filter(r => 
                (r.Programa === 'Alérgenos') || 
                (r.Programa === 'Control de Alérgenos') ||
    ...
    const handleOpenCreateACPM = (rep) => {
        setAcpmData({
            origen: `Control Alérgenos #${rep.ID_Reporte}`,
            descripcion: `Posible contaminación cruzada o error etiqueta: ${rep.Observaciones || 'Sin detalles'}`,
            ...
            <div className="page-header">
                <div>
                    <h1 className="page-title" style={{display:'flex', alignItems:'center', gap:'10px'}}>
                        <FaBreadSlice style={{color: THEME_COLOR}}/> Control de Alérgenos
                    </h1>
                    <p style={{color:'#64748b', marginTop:'-5px'}}>Gestión de contaminación cruzada, etiquetado y limpiezas especiales.</p>
                </div>
            </div>
                ...
                <div className="fade-in">
                    ...
                    <div className="table-container">
                        <table className="modern-table">
                            <thead>
                                <tr><th>Fecha</th><th>Responsable</th><th>Formato / Línea</th><th>Estado</th><th style={{textAlign:'right'}}>Acciones</th></tr>
                            </thead>
                            <tbody>
                                {loadingReportes ? <tr><td colSpan="5" style={{textAlign:'center', padding:'2rem'}}>Cargando...</td></tr> : 
                                getFilteredReportes().length === 0 ? <tr><td colSpan="5" style={{textAlign:'center', padding:'2rem', color:'#94a3b8'}}>No hay registros de alérgenos.</td></tr> :
                                getFilteredReportes().map(rep => (
                                    <tr key={rep.ID_Reporte} className="modern-row">
                                        ...
                                        <td className="modern-cell">
                                            <div className="asset-name">{rep.Activo || 'Línea General'}</div>
            ...
            {showFileManager && (
                <div className="modal-overlay">
                    <div className="modal-content" style={{width: '95%', maxWidth: '1100px', height: '85vh', padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column'}}>
                        <FileManager initialFolderId={currentCategory.id} title={`Alérgenos: ${currentCategory.name}`} onClose={() => setShowFileManager(false)} />
                    </div>
                </div>
            )}
                        ...
                        <form onSubmit={handleCreateCard}>
                            <div className="modal-body">
                                ...
                                <div className="form-group">
                                    ...
                                    <input className="form-control" required value={newCardData.titulo} onChange={e=>setNewCardData({...newCardData, titulo:e.target.value})} placeholder="Ej: Fichas Técnicas 2026" />
                                </div>
